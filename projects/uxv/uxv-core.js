...
(function () {
  ...
  var themeListeners = [];
  UXV.onThemeChange = function (fn) { themeListeners.push(fn); };
  ...
  UXV.reducedMotion = rmq.matches;
  if (rmq.addEventListener) rmq.addEventListener("change", function (e) { UXV.reducedMotion = e.matches; });

  /* ---------- animation loop manager ---------------------------
     UXV.raf(el, drawFn) runs drawFn(tMs) every frame while `el`
  ...
  var ioFallbacks = [];
  UXV.__markIO = function () { UXV._ioFired = true; };
  UXV.onIOBroken = function (fn) { ioFallbacks.push(fn); };
  setTimeout(function () {
    if (!UXV._ioFired) ioFallbacks.forEach(function (fn) { try { fn(); } catch (e) {} });
  }, 1600);

  /* ---------- deterministic randomness ------------------------- */

  UXV.rng = function (seed) {
    var a = seed >>> 0;
    function next() {
  ...
  UXV.clamp = function (x, a, b) { return Math.max(a, Math.min(b, x)); };
  UXV.lerp = function (a, b, t) { return a + (b - a) * t; };
  UXV.ease = function (t) { return t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2; };
  UXV.dist = function (x1, y1, x2, y2) { var dx = x2 - x1, dy = y2 - y1; return Math.sqrt(dx * dx + dy * dy); };

  /* ---------- canvas sizing (HiDPI) ----------------------------
     Fixes the canvas backing store to CSS size × devicePixelRatio
     and returns the 2D context scaled to CSS pixels.             */

  UXV.fitCanvas = function (canvas, cssH) {
  ...
  UXV.svg = function (tag, attrs, parent) {
  ...
  UXV.tooltip = {
    show: function (html, x, y) {
      ...
      var left = UXV.clamp(x + pad, 6, window.innerWidth - w - 6);
  ...
  UXV.tabs = function (root, onChange) {
    if (!root) return;
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var toc = document.querySelectorAll(".uxv-toc a");
    var byId = {};
    ...
    document.querySelectorAll(".uxv-section").forEach(function (s) { spy.observe(s); });

    /* gentle scroll reveal on widgets */
    var widgets = document.querySelectorAll(".uxv-widget, .uxv-card");
    if (!UXV.reducedMotion) {
      ...
      widgets.forEach(function (w) { w.classList.add("uxv-reveal"); rev.observe(w); });
      UXV.onIOBroken(function () {
    ...
    if (bb) bb.addEventListener("click", function () {
      setTimeout(function () {
        ...
        if (box) box.style.outline = "2px solid " + UXV.colors().s1;
